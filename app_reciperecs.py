...
def find_similar_recipes(sampled_data, user_input, num_similar=5):
    ...
    # Validate dimensions
    if combined_embeddings.shape[0] != len(sampled_data):
        st.error("Error: The size of embeddings does not match the size of the data.")
        return []
    ...
    # Ensure the number of features matches with combined_embeddings
    ...
    # Validate that indices exist in sampled_data
    similar_recipes = [idx for idx in similar_recipes if idx < len(sampled_data)]

    if not similar_recipes:
        st.error("No similar recipes found. Please try different input.")
        return []

    # Retrieve similar recipe names
...
st.title('Recipe Recommendation System :pancakes:')
st.write('**Gopal**')
st.write("Discover and try new recipes :arrow_down:")
st.divider()
